...
const Navbar: React.FC<NavbarProps> = ({ onNavigate }) => {
  ...
  return (
    <>
      <nav
        className={`fixed top-0 w-full z-[100] transition-all duration-500 select-none ${
          isScrolled || mobileMenuOpen
            ? 'bg-black/95 backdrop-blur-xl border-b border-white/10 py-2 md:py-3'
            : 'bg-transparent py-8 md:py-10'
        }`}
      >
        <div className="max-w-[1440px] mx-auto px-6 lg:px-12">
          <div className="flex justify-between items-center">
            {/* MASSIVE UPPER LEFT LOGO - FIXED SIZE */}
            <div 
              onClick={handleLogoClick}
              className="flex-shrink-0 flex items-center cursor-pointer group transition-transform active:scale-95 z-[110]"
            >
              <Logo className={`w-auto transition-all duration-500 ${isScrolled ? 'h-7 md:h-9' : 'h-10 sm:h-12 md:h-14'}`} />
            </div>
            ...
            <div className="hidden lg:flex items-center space-x-12">
              ...
              <div className="flex items-center space-x-6">
                <button
                  ...
                  className={`group relative flex flex-col items-start bg-white/5 hover:bg-brand-500/10 border border-brand-500/30 hover:border-brand-500 transition-all ${isScrolled ? 'px-4 py-2 rounded-xl' : 'px-6 py-3.5 rounded-2xl'}`}
                >
                  <span className={`flex items-center gap-2 font-black text-brand-500 uppercase tracking-[0.25em] transition-all ${isScrolled ? 'text-[8px] mb-0.5' : 'text-[10px] mb-1'}`}>
                    <Building2 className={isScrolled ? "w-3 h-3" : "w-3.5 h-3.5"} /> Studio Owners Only
                  </span>
                  <span className={`text-white font-black uppercase tracking-tight leading-tight group-hover:text-brand-500 transition-colors ${isScrolled ? 'text-sm' : 'text-xl'}`}>
                    Partner With Us
                  </span>
                </button>
                ...
                <button 
                  ...
                  className={`bg-brand-500 hover:bg-brand-400 text-black font-black uppercase tracking-widest shadow-[0_0_30px_rgba(171,226,37,0.3)] hover:shadow-[0_0_60px_rgba(171,226,37,0.6)] transform hover:-translate-y-1 transition-all ${isScrolled ? 'px-6 py-2.5 rounded-xl text-xs' : 'px-10 py-4.5 rounded-2xl text-sm'}`}
                >
                  Download App
                </button>
              </div>
